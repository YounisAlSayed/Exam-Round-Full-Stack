...
class TeacherCourses
{
    ...
    {
        $sql = "SELECT * FROM teacher_courses WHERE teacher_id = :teacher_id AND course_id = :course_id";
        $statement = $this->pdo->prepare($sql);
        $statement->execute(['teacher_id' => $teacher_id, 'course_id' => $courses_id]);
    ...
    public function create(int $teacher_id, int $courses_id): bool
    {
        ...
        $statement = $this->pdo->prepare($sql);
        return $statement->execute(['teacher_id' => $teacher_id, 'course_id' => $courses_id]);
    }
