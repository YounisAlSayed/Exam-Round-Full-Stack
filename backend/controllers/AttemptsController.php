...
class AttemptsController
{
    ...
    public function getStudentAttempt($student_id)
    {
        ...
        $attempts = $this->attempts->getByStudent($student_id);
    ...
    public function updateStudentAttempt($student_id)
    {
        ...
        $existingAttempt = $this->attempts->findByExamAndStudent($exam_id, $student_id);
        ...
        if ($action === 'start') {
            ...
            $attemptId = $this->attempts->start($exam_id, $student_id);
        ...
        if (!$this->attempts->submit((int) $existingAttempt['id'])) {
            http_response_code(500);
            return new ViewModel('attempts/list', ['error' => 'Internal Server Error', 'student_id' => $student_id]);
        }
    ...
    public function deleteStudentAttempt($attempt_id)
    {
        ...
        if (!$this->attempts->delete($attempt_id)) {
            http_response_code(500);
